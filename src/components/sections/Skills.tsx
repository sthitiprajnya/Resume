...
// Generate flat list of tags for marquee - hoisted to avoid re-creation on every render
const MARQUEE_TAGS = [
...
export function Skills() {
  return (
    <section id="skills" className="py-24 bg-black overflow-hidden relative">
      ...
      {/* Marquee Tags */}
      <div
        className="relative w-full border-y border-border bg-deep py-6 overflow-hidden flex flex-col space-y-4 group/marquee"
        role="region"
        aria-label="Scrolling tech tags"
        tabIndex={0}
      >
        ...
        <div className="flex w-max animate-marquee-right">
          {[...REVERSED_TAGS, ...REVERSED_TAGS].map((tag, i) => (
            <div
              key={`row2-${i}`}
              className="mx-3 px-4 py-1.5 rounded-sm bg-surface border border-border font-mono text-[0.75rem] text-text-secondary whitespace-nowrap"
            >
      ...
      <style>{`
        ...
        .group\\/marquee:focus-within .animate-marquee-left,
        .group\\/marquee:focus-within .animate-marquee-right {
