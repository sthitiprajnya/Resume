...
import { PERSONAL, HERO_ROLES, HERO_STATS } from '@/data/portfolio';
...
export function Hero() {
  ...
  return (
    <section id="hero" className="relative min-h-screen flex items-center justify-center pt-16 bg-[var(--gradient-hero)] overflow-hidden">
      ...
      <div className="relative z-10 w-full max-w-[900px] px-6 flex flex-col items-center text-center">
        ...
        >
          <CyberButton onClick={() => scrollTo('projects')}>
            VIEW_PROJECTS
          </CyberButton>
          <CyberButton color="green" onClick={() => scrollTo('contact')}>
            CONTACT_ME
          </CyberButton>
          ...
          <div className="flex space-x-4 pt-4 sm:pt-0 sm:ml-4">
            <a
              ...
              aria-label="LinkedIn"
              className="p-3 border border-border rounded-sm text-text-secondary hover:text-cyan hover:border-cyan hover:shadow-[var(--glow-cyan-sm)] transition-all"
            >
      ...
      <motion.button
        ...
        className="absolute bottom-8 left-1/2 -translate-x-1/2 text-cyan flex flex-col items-center group p-4 focus-visible:outline-2 focus-visible:outline-cyan"
        aria-label="Scroll to About section"
      >
        <span className="font-mono text-[0.6rem] uppercase tracking-widest mb-2 opacity-50 group-hover:opacity-100 transition-opacity">SCROLL</span>
        <svg
