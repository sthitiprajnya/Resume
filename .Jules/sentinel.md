...
## 2025-05-28 - [Supply Chain Vulnerabilities in Build Tools]
**Vulnerability:** High-severity command injection in `glob` and moderate XSS in `postcss` dependencies.
**Learning:** Even static sites can be vulnerable during the build phase (CI/CD) or through runtime artifacts generated by build-time dependencies like PostCSS. Automated audit tools are essential for identifying hidden risks in deeply nested dependency trees.
**Prevention:** Use `pnpm overrides` to force-upgrade transitive dependencies to patched versions when parent packages haven't yet updated their own dependency requirements.
